...
window.esShowLogin=msg=>{
  ...
  <p class="err text-sm text-error mt-3 text-center min-h-[20px]" role="alert">${esc(msg||'')}</p><button type="button" class="forgot block mx-auto text-sm text-primary font-medium hover:underline">Forgot password?</button></div></form>
  ...
  const f=d.querySelector('form'),b=f.querySelector('button.relative'),err=f.querySelector('.err'),fb=f.querySelector('.forgot');f.email.focus();
  ...
  const tick=esReset.bind(fb);
  fb.onclick=async()=>{err.classList.replace('text-green-700','text-error');const r=await esReset.send(ES.resetPassword,f.email.value);err.textContent=r.msg;if(r.ok)err.classList.replace('text-error','text-green-700');tick()};
