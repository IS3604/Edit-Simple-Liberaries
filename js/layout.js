...
function header(){
  ...
  const link=([h,t,i,a])=>`<a href="${h}" ${a?'aria-current="page"':''} class="flex items-center gap-1.5 h-9 px-3.5 rounded-full text-sm font-semibold whitespace-nowrap transition ${a?'bg-white text-primary shadow-sm':'text-white/90 hover:bg-white/15 hover:text-white'}"><span class="material-symbols-outlined !text-[18px]">${i}</span>${esc(t)}</a>`;
  const html=`<div class="max-w-7xl mx-auto px-4 md:px-6 h-16 flex items-center gap-4">
<a href="index.html" class="flex items-center gap-2.5 shrink-0" aria-label="Edit Simple Libraries home"><img src="logo.png" alt="" width="36" height="36" class="h-9 w-9 rounded-lg object-cover ring-2 ring-white/30"><span class="font-display font-bold text-white text-[17px] leading-tight hidden sm:block">Edit Simple<span class="block text-[11px] font-medium text-white/75 tracking-wide uppercase">Libraries</span></span></a>
<nav class="hidden lg:flex items-center gap-1 mx-auto">${nav.map(link).join('')}</nav>
<div class="flex items-center gap-2 ml-auto lg:ml-0"><button id="es-search" class="flex items-center gap-2 h-9 pl-3 pr-2 rounded-full bg-white/15 hover:bg-white/25 text-white text-sm transition" aria-label="Search videos"><span class="material-symbols-outlined !text-[20px]">search</span><span class="hidden md:inline text-white/85 pr-1">Search videos</span><kbd class="hidden md:inline text-[10px] font-semibold bg-white/20 rounded px-1.5 py-0.5">⌘K</kbd></button>
<button id="es-menu" class="lg:hidden h-9 w-9 grid place-items-center rounded-full bg-white/15 hover:bg-white/25 text-white" aria-label="Open menu" aria-expanded="false"><span class="material-symbols-outlined">menu</span></button></div></div>
<nav id="es-mnav" class="hidden lg:hidden border-t border-white/15 px-4 py-3 flex-col gap-1">${nav.map(link).join('')}</nav>`;
  let h=document.getElementById('es-header');
  if(!h){h=document.createElement('header');h.id='es-header';h.className='sticky top-0 z-50 w-full bg-primary shadow-[0_2px_12px_rgba(240,127,46,.25)]';document.body.prepend(h)}
  h.innerHTML=html;
